...
class ConversationStateMachine:
    ...
    def __init__(self, config: dict):
        ...
        # B2.3: real barge-in — sustained Silero VAD speech confidence
        # instead of a raw RMS energy threshold, which music/TV/keyboard
        # noise could trip just as easily as an actual voice.
        self._barge_confidence_threshold = barge_cfg.get("confidence_threshold", 0.7)
        self._barge_min_ms = barge_cfg.get("min_duration_ms", 300)
        # B2.4: ambient mode (TV/music playing) raises the bar further —
        # applied on top of the base threshold/duration above.
        self._barge_ambient_confidence_boost = barge_cfg.get("ambient_confidence_boost", 0.15)
        self._barge_ambient_min_ms_boost = barge_cfg.get("ambient_min_duration_boost_ms", 200)
    ...
    def set_ambient_active(self, active: bool):
        """Mirror of WakeWordDetector.set_ambient_active — raises the
        barge-in bar while music/TV is playing (B2.4)."""
        self._ambient_active = active

    def check_barge_in(self, speech_confidence: float) -> bool:
        """Check if sustained speech confidence during SPEAKING constitutes
        a barge-in (B2.3 — Silero VAD probability, not raw RMS energy, so
        music/TV/keyboard noise no longer interrupts Sara).
        ...
        confidence_threshold = self._barge_confidence_threshold
        min_ms = self._barge_min_ms
        if self._ambient_active:
            confidence_threshold += self._barge_ambient_confidence_boost
            min_ms += self._barge_ambient_min_ms_boost

        now = time.monotonic()

        if speech_confidence >= confidence_threshold:
            if self._barge_in_start is None:
                self._barge_in_start = now
            elif (now - self._barge_in_start) * 1000 >= min_ms:
