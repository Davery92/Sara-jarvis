...
class EventReporter:
    ...
    async def report_watchdog_paused(self, reason: str):
        """Report a conversation-watchdog trip (B2.7 loop breaker of last
        resort) — the backend turns this into a desktop toast."""
        return await self._backend.send_event(
            f"{self._sensory_base}/voice/watchdog-paused",
            {
                "reason": reason,
                "source": "jetson_voice",
            },
