 ...
 * frontend/src/types/foodContracts.ts. Field-name parity across all three is
 * checked by ios-app/scripts/check-food-contract-parity.mjs — change a field
 * here, change it in both other copies and the fixtures file, or the parity
 * script fails.
 *
 * v1 rows (the informal shape documented above `FoodLogCreate` in
 * backend/app/routes/fitness.py — see `FoodLog.detailed_item` in fitness.ts)
 * are NOT rewritten. This is additive: existing endpoints keep accepting
 * untyped v1 dicts.
 */
