...
class ContextLayer:
    ...
    def __init__(self):
        ...
        self._fast_model = llm_config.fast_model
    ...
    async def inject_predictions(self, user_id: str):
        """
        Inject forward-looking predictions into the context layer.
        Called after predictions are generated by the predictive engine.
        """
        try:
            from app.services.predictive_engine import generate_predictions
            ...
            for pred in predictions[:4]:
                ...
                lines.append(f"- {icon} {pred['message']}")

            current = self._read_layer(user_id)
            # Remove old predictions section if present
            import re
            current = re.sub(r'\n\n\*\*Predictions & Suggestions:\*\*.*?(?=\n\n\*\*|\Z)', '', current, flags=re.DOTALL)

            self._write_layer(user_id, current.rstrip() + "\n".join(lines))
            logger.info(f"💡 Injected {len(predictions)} predictions into context layer")
        except Exception as e:
            logger.debug(f"Prediction injection failed: {e}")
