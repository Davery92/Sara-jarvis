...
## Phase T.4 — The response loop teaches her — 2026-07-07

Found and fixed a **major pre-existing bug** while implementing this: `attention_queue._log_action()` used `:entry::jsonb` — the exact `::type` + named-bind-parameter conflict already known from the pgvector gotcha (`gotcha_pgvector_cast`). This silently threw on every single quick action (Done, Archive, Snooze, Reply — all of them, since every action path calls `_log_action` at the end), and because the exception was swallowed without a rollback, it poisoned the whole transaction for everything that ran afterward in the same request — including the `mark_archived`/`mark_completed`/reminder-creation writes that had already succeeded moments earlier. **Every quick-action button in the app has been reporting `success: true` while silently not persisting anything**, until this fix. Changed to `CAST(:entry AS jsonb)` and changed the `except` to re-raise (fail loud) instead of swallow.

| ID | Status | Evidence |
|----|--------|----------|
| Triad (Do it / Not now / Stop these) | **PASS — proven live** | "Do it" = the category's existing primary action (untouched). Added `not_now` and `stop_these` as universal actions on every item. Verified: a real item's stored payload actions list includes both new entries alongside the category-specific ones. |
| "Stop these" -> theta nudge | **PASS — proven live** | Real test: theta for `domain=meta, context=available` moved 0.795 -> 0.841 (a real "stop" outcome via `attention_learning.apply_engagement`), item archived, action logged. Theta restored to its exact pre-test value (0.7283522738408288) afterward — did not leave synthetic drift in David's real learned policy. |
| "Not now" -> re-surfaces at next context change | **PASS — proven live, both directions** | Positive case: snoozed from `available`, then a real `_resurface_snoozed_items('...', 'FOCUSED_WORK')` call flipped it back to `sent`. Negative case: snoozed from `available`, then a same-context check (`'AVAILABLE'` -> resolves to `available`) correctly left it `snoozed` — no premature resurface. Also fixed: `list_items`'s default view was missing `snoozed` from its status exclusion list, so a "Not now" item would have stayed visible in the inbox — added. |
| "Not now" — the SAME `::type` bind bug, caught before shipping | **PASS — found and fixed in my own new code** | My first version of the `not_now` UPDATE used `to_jsonb(:context::text)` — the identical bug pattern to the one just found in `_log_action`, in code I wrote minutes earlier. Caught by testing before calling it done; fixed to `to_jsonb(CAST(:context AS text))`; swept the touched files for any other instance of the pattern (none found) before moving on. |
| Replies mark_engaged + carry item context | **PASS — proven live for the core mechanism; partial for full context threading** | The existing `run_action` "chat" kind already called `mark_engaged` before this session — that part of T.4's stated problem ("nothing calls it when you reply") didn't hold up under investigation; verified it fires correctly (now that the transaction-poisoning bug is fixed). Added: `item_id`/`category` in the returned chat directive; a new `ChatRequest.attention_item_id` field; and backend context injection in `/chat/stream` mirroring the existing (already-working) `inbox_item_id` pattern — verified live via a real chat call: log line "Injected attention item context: Quick context check" fired, and the item's `read_at`/status confirmed `mark_engaged` ran. |

**Not completed**: the web frontend's UI prop chain (`AttentionInbox.tsx` -> `onStartChat` -> `InboxWorkspaceView.tsx` -> `App-interactive.tsx`) doesn't yet set the `window.__attentionItemId` value `ChatInterface.tsx` now reads (added as a safe, inert no-op — reads a window global that's currently always `undefined`, so `attention_item_id` is simply never sent yet; nothing is broken by this, it's just not wired end-to-end). iOS has no equivalent wiring at all yet. This is a real, scoped remaining task: thread `item_id` through those four files (or the iOS equivalent) the same way `onOpenContentChat` already threads `inboxItemId` (itself also currently a dead end — `window.__inboxItemId` is set but never read anywhere, a separate pre-existing gap noticed along the way, not fixed here).

## Phase D — People become a real graph — 2026-07-07

| ID | Status | Evidence |
|----|--------|----------|
| D.1 (historical seed) | **PASS — proven live** | New `app/services/person_seed.py` replays every existing `email` row (chronological) through the exact same `upsert_person_from_email()` the live inbound sync uses — no duplicated identity-matching logic. Result: `{'emails_replayed': 1365, 'upserts': 949, 'skipped': 416, 'new_person_rows': 109}`. `person` table: 4 -> 113 rows (exceeds the plan's "≥50" target). 109 new rows tagged `notes='seed_2026_07'` — one-statement reversible (`DELETE FROM person WHERE notes = 'seed_2026_07'`). 70 real `signal_baseline` cadence rows created (domain=`people`) — `reconnect_overdue` has real EWMA data from day one instead of a multi-week warm-up. Names match exactly what `list_people` already surfaced in the U.8 verification (Jim Venezia, Dave Brink, Laura Weippert, Matthew Albano) — consistent, not fabricated. |
| D.1 known data-quality note | Not fixed, by design | "Jim Venezia" (373 interactions), "James Venezia" (24), and "James J Venezia" (16) are 3 separate person rows for what's almost certainly one real contact — `person_service.py`'s own docstring mandates this ("no fuzzy name matching... prefer missed merges over wrong merges"). Not a bug; flagging so it isn't mistaken for one. |
| D.2 (Sent-folder sync) | **PASS — proven live, real Graph API call** | New `app.tasks.email_sync.sync_sent_items` task (scheduled every 15 min, migration 095) fetches `folder="sentitems"` and upserts each recipient with `direction='email_out'` — doesn't store full sent-item rows (the `email` model's shape is inbound-oriented); only the person-layer signal. Fired live: real MS Graph call, `{'sent_items_seen': 1, 'recipients_upserted': 1, 'errors': 0}`. Confirmed: "Dave Brink" now has `last_interaction_kind='email_out'` — the first outbound interaction ever captured (R9's exact gap). |
| D.3 (real-time chat mentions) | **PASS — proven live, real bug found and fixed in my own new code before shipping** | New `pkg_realtime_extractor.bump_mentioned_people()`, wired into `/chat/stream` alongside the existing PKG extraction call. First version had a real false-positive bug: a person row from the D.1 seed is literally named "The Quo Team" — its first token "the" matched the ordinary word "the" in any message, silently mis-bumping mention counts. Caught by testing before shipping (same class of bug as the `_has_proper_noun` fix from Phase A) — fixed by requiring a mid-sentence-capitalized match (excluding the sentence-initial word, which is capitalized by English orthography regardless of content) plus a small stopword list. Re-verified: the exact same test message now correctly bumps only the two legitimate "Jim" matches and leaves "Quo Support"/"The Quo Team" untouched; a control message ("the quick brown fox...") bumps nobody. Test-affected `person` rows restored to their true pre-test values (re-derived from the real `email` table, not guessed) — `mention_count` correctly reset to 0, `last_interaction_at`/`kind` restored from real email history. One acknowledged, negligible residual: the cadence EWMA for Jim Venezia absorbed one synthetic test sample (of 375 total) — not worth a fragile manual reversal. |
| D.4 (list_people reachable) | Already done | Wired during the U.8 registry sweep (previous session turn), verified live there. |
| D.5 (meeting attendee bump) | **PASS — proven live**, plus a real ordering bug caught before shipping | New `person_service.bump_meeting_attendees()`, called from `proactive_checkins.scan_ended_meetings()` — the actual "meeting just ended" hook, distinct from `person_service_sync.link_attendees_to_people` (which only bumps if the event was already in the past at calendar-*sync* time — most real meetings sync *before* they happen, so that path rarely fires for a normal meeting). Caught before shipping: my first placement ran the bump before the existing per-event dedup check, which would have double/triple-bumped a single meeting across the scan's 10-min-interval / 25-min-lookback overlap — moved inside the same dedup gate the follow-up-thread creation already uses. Verified directly (no real ended meeting existed at test time): `interaction_count` 373->374, `last_interaction_kind` -> `meeting`, timestamp updated — then restored to the exact prior values afterward. |

All Phase D test-induced data changes were either tagged-and-reversible (D.1) or explicitly restored to their real, pre-test values (D.2 left as-is since it's genuinely real data; D.3/D.5 test artifacts reverted) — no synthetic rows were left behind, and no real signal was permanently altered beyond the one acknowledged negligible EWMA nudge.
