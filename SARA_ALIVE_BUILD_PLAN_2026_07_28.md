...
5.4 **Graph hygiene:** execute the audited Neo4j ActionItem purge (425k bloat); keep Neo4j for relations, pgvector for retrieval, both behind the recall door. **Purge itself already DONE** (2026-06-10, prior session) — verified live 2026-07-29: `MATCH (n:ActionItem)` and `MATCH (n:TemporalInfo)` both count **0**, and the writer is confirmed disabled (`enhanced_neo4j_schema.py` — "TemporalInfo/ActionItem nodes intentionally not stored... cleaned up 2026-06"). It hasn't regrown. Remaining Arc 5.4 scope is only "both behind the recall door," which is 5.1's work, not a separate purge task.
