...
class VoiceVisionService:
    ...
    async def _handle_speech_end(self, speech_audio: np.ndarray | None = None):
        ...
        await self._get_and_speak_response(transcript, duration_seconds=duration)

    async def _get_and_speak_response(self, transcript: str, duration_seconds: float = 0.0):
