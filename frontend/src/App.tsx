...
function App() {
  ...
  // useShellAuth re-fetches /auth/me whenever this callback's identity
  // changes (its internal checkAuth is keyed on it). Inline arrow functions
  // here would get a fresh identity on every App render, re-triggering that
  // fetch on every unrelated state update anywhere in this large component —
  // amplified by anything that renders frequently (e.g. a live-updating
  // clock) into a request flood that starves the rest of the API. Must stay
  // memoized.
  const handleSessionStart = useCallback((nextView: AppView, options?: { resetChat: boolean }) => {
    navigateToView(nextView)
    if (options?.resetChat) {
      setChatMessages(getInitialChatMessages())
    }
  }, [navigateToView, getInitialChatMessages])
      ...
      <div className="flex flex-col md:flex-row md:space-x-6 flex-1 min-h-0">
        ...
        <main ref={mainContentRef} className="flex-1 min-w-0 flex flex-col min-h-0 overflow-hidden">
          ...
          <ShellWorkspaceContent
            ...
            onVerificationAnswer={answerVerification}
